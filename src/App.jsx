...
export const CYLINDER_PRESETS = [
  ...
  { id: '6kg',  label: '6 kg',  net_g:  6000, tare_g:  8800 }, // ← fixed from 8000
...
// ─── weightToPercent ───────────────────────────────────────────────────────
// ESP32 always sends TOTAL weight (cylinder body + gas inside)
// We subtract the empty cylinder tare to get gas remaining, then convert to %
const weightToPercent = (weight_g, preset, customTare_g = null) => {
  if (weight_g == null || !preset) return 0
  const w = parseFloat(weight_g)
  if (isNaN(w) || w <= 0) return 0

  // Use manually entered tare if available, otherwise use preset
  const tare = customTare_g != null ? parseFloat(customTare_g) : preset.tare_g

  // Subtract empty cylinder body weight to get gas remaining
  const gasRemaining = w - tare

  // If negative or zero, cylinder is empty
            ...
            {data.map((d, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end', minWidth: 0 }}>
                <div style={{
                  ...
                  minHeight: d.value > 0 ? 3 : 0, background: color,
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: barAreaHeight, width: '100%', borderBottom: '1px solid var(--border)' }}>
            {data.map((d, i) => {
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end', minWidth: 0 }}>
                  <div style={{
                    width: '100%', display: 'flex', gap: 3, alignItems: 'flex-end',
                    justifyContent: 'center', height: barHeightPercent,
                    minHeight: (d.high > 0 || d.low > 0) ? 3 : 0,
                  }}>
                    <div style={{
                      flex: 1, borderRadius: '2px 2px 0 0',
                      height: maxBarHeight > 0 ? `${(d.high / maxBarHeight) * 100}%` : '0%',
                      minHeight: d.high > 0 ? 3 : 0, background: '#ff4560',
                      opacity: d.high > 0 ? 1 : 0.12, position: 'relative',
                      transition: 'height 0.6s cubic-bezier(.4,0,.2,1)'
                    }}>
                      {showValues && d.high > 0 && (
                        <div style={{
                          position: 'absolute', top: -20, left: '50%', transform: 'translateX(-50%)',
                          fontFamily: 'var(--font-mono)', fontSize: 8, color: '#ff4560', fontWeight: 600,
                          whiteSpace: 'nowrap', background: 'rgba(0,0,0,0.7)',
                    ...
                    <div style={{
                      ...
                      minHeight: d.low > 0 ? 3 : 0, background: '#ffb020',
                      opacity: d.low > 0 ? 1 : 0.12, position: 'relative',
                      transition: 'height 0.6s cubic-bezier(.4,0,.2,1)'
                    }}>
                      {showValues && d.low > 0 && (
                        <div style={{
                          position: 'absolute', top: -20, left: '50%', transform: 'translateX(-50%)',
                          fontFamily: 'var(--font-mono)', fontSize: 8, color: '#ffb020', fontWeight: 600,
                          whiteSpace: 'nowrap', background: 'rgba(0,0,0,0.7)',
                          padding: '2px 4px', borderRadius: 4, pointerEvents: 'none',
                        }}>
                          {d.low}
